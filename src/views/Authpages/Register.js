...
import {setNotifyFalse} from './authSlice';
...
export default function Register({navigation}) {
  ...
  const {data, loading, error, registerData} = useSelector(state => state.auth);
      ...
      if (registerData?.status) {
        ...
        try {
          const companyPayload = {
            ...
            phone: 9876541230,
            user_id: registerData?.data?.user?.id,
          };
          const createComapany = await LoginAPIs.createCompany(companyPayload);
        } catch (error) {
          console.log(error);
        }
