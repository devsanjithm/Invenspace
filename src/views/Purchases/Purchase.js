...
import {getPurchaseDetails} from './purchaseAction';
...
export default function Purchase({navigation}) {
  ...
  function getData() {
    ...
    if (_.isString(userId)) {
      dispatch(getPurchaseDetails(userId));
    } else {
  ...
  function search(text) {
    ...
    const newData = data?.data.filter(item => {
      const itemData = `${item?.purchase_desc?.toLowerCase()}${item?.purchase_total}${item?.purchase_id}${
        item?._id
      }`;
  ...
  return (
    <>
      ...
      <SafeAreaView
        ...
        <AppHeaders title={'Purchases'} color={'#fff'} main={true}>
          {handleSearchUIState ? (
                ...
                <View
                  ...
                  <View style={{flexDirection:'row'}}>
                    ...
                    <View style={{paddingHorizontal: 20}}>
                      <Text style={globalStyles}>{ele?. purchase_id}</Text>
                      <Text style={globalStyles}>{ele?. purchase_desc}</Text>
                    </View>
                  </View>
                  <View>
                    <Pressable
